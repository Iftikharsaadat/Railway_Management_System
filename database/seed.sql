...
SELECT t.train_name, c.coach_name, se.seat_number, sc."date", se.reservation_status
FROM train t
JOIN coach c ON c.train_id = t.train_id
JOIN seat se ON se.coach_id = c.coach_id
JOIN schedule sc ON sc.route_id = t.route_id
WHERE t.train_id = 5
AND sc."date" = '2026-08-15'
;
